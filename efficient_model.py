...
class EfficientPolynomialSearch_4_2(nn.Module):
    """
    Polynomial decomposition using PyTorch.
    ...
    def __init__(self):
        ...
        Args:
            degree (int): Degree of the polynomial.
        """
        super(EfficientPolynomialSearch_4_2, self).__init__()
        # Set slack values for compilation
        degree, deg_q = 16, 4
        # Set the degree of R
        self.degree = 4
    ...
    def forward(self) -> torch.Tensor:
        ...
        # Update the output by the polynomial
        output[0] = self.P[0] + self.P[1]*self.Q[0] + self.P[2]*self.Q[0]**2
        output[1] = self.P[1]*self.Q[1] + 2*self.P[2]*self.Q[0]*self.Q[1]
        output[2] = self.P[1]*self.Q[2] + 2*self.P[2]*self.Q[0]*self.Q[2] + self.P[2]*self.Q[1]**2
        output[3] = 2*self.P[2]*self.Q[1]*self.Q[2]
        output[4] = self.P[2]*self.Q[2]**2
...
class EfficientPolynomialSearch_9_3(nn.Module):
    """
    Polynomial decomposition using PyTorch.
    ...
    def __init__(self):
        ...
        Args:
            degree (int): Degree of the polynomial.
        """
        super(EfficientPolynomialSearch_9_3, self).__init__()
        # Set slack values for compilation
        degree, deg_q = 16, 4
        # Set the degree of R
        self.degree = 9
    ...
    def forward(self) -> torch.Tensor:
        ...
        # Update the output by the polynomial
        output[0] = self.P[0] + self.P[1]*self.Q[0] + self.P[2]*self.Q[0]**2 + self.P[3]*self.Q[0]**3
        output[1] = self.P[1]*self.Q[1] + 2*self.P[2]*self.Q[0]*self.Q[1] + 3*self.P[3]*self.Q[0]**2*self.Q[1]
        output[2] = self.P[1]*self.Q[2] + 2*self.P[2]*self.Q[0]*self.Q[2] + self.P[2]*self.Q[1]**2 + 3*self.P[3]*self.Q[0]**2*self.Q[2] + 3*self.P[3]*self.Q[0]*self.Q[1]**2
        output[3] = self.P[1]*self.Q[3] + 2*self.P[2]*self.Q[0]*self.Q[3] + 2*self.P[2]*self.Q[1]*self.Q[2] + 3*self.P[3]*self.Q[0]**2*self.Q[3] + 6*self.P[3]*self.Q[0]*self.Q[1]*self.Q[2] + self.P[3]*self.Q[1]**3
        output[4] = 2*self.P[2]*self.Q[1]*self.Q[3] + self.P[2]*self.Q[2]**2 + 6*self.P[3]*self.Q[0]*self.Q[1]*self.Q[3] + 3*self.P[3]*self.Q[0]*self.Q[2]**2 + 3*self.P[3]*self.Q[1]**2*self.Q[2]
        output[5] = 2*self.P[2]*self.Q[2]*self.Q[3] + 6*self.P[3]*self.Q[0]*self.Q[2]*self.Q[3] + 3*self.P[3]*self.Q[1]**2*self.Q[3] + 3*self.P[3]*self.Q[1]*self.Q[2]**2
        output[6] = self.P[2]*self.Q[3]**2 + 3*self.P[3]*self.Q[0]*self.Q[3]**2 + 6*self.P[3]*self.Q[1]*self.Q[2]*self.Q[3] + self.P[3]*self.Q[2]**3
        output[7] = 3*self.P[3]*self.Q[1]*self.Q[3]**2 + 3*self.P[3]*self.Q[2]**2*self.Q[3]
        output[8] = 3*self.P[3]*self.Q[2]*self.Q[3]**2
        output[9] = self.P[3]*self.Q[3]**3
...
class EfficientPolynomialSearch_25_5(nn.Module):
    """
    Polynomial decomposition using PyTorch.
    ...
    def __init__(self):
        ...
        Args:
            degree (int): Degree of the polynomial.
        """
        super(EfficientPolynomialSearch_25_5, self).__init__()
        # Set slack values for compilation
        degree, deg_q = 16, 4
        # Set the degree of R
        self.degree = 25
    ...
    def forward(self) -> torch.Tensor:
        ...
        output[2] = self.P[1]*self.Q[2] + 2*self.P[2]*self.Q[0]*self.Q[2] + self.P[2]*self.Q[1]**2 + 3*self.P[3]*self.Q[0]**2*self.Q[2] + 3*self.P[3]*self.Q[0]*self.Q[1]**2 + 4*self.P[4]*self.Q[0]**3*self.Q[2] + 6*self.P[4]*self.Q[0]**2*self.Q[1]**2 + 5*self.P[5]*self.Q[0]**4*self.Q[2] + 10*self.P[5]*self.Q[0]**3*self.Q[1]**2
        output[3] = self.P[1]*self.Q[3] + 2*self.P[2]*self.Q[0]*self.Q[3] + 2*self.P[2]*self.Q[1]*self.Q[2] + 3*self.P[3]*self.Q[0]**2*self.Q[3] + 6*self.P[3]*self.Q[0]*self.Q[1]*self.Q[2] + self.P[3]*self.Q[1]**3 + 4*self.P[4]*self.Q[0]**3*self.Q[3] + 12*self.P[4]*self.Q[0]**2*self.Q[1]*self.Q[2] + 4*self.P[4]*self.Q[0]*self.Q[1]**3 + 5*self.P[5]*self.Q[0]**4*self.Q[3] + 20*self.P[5]*self.Q[0]**3*self.Q[1]*self.Q[2] + 10*self.P[5]*self.Q[0]**2*self.Q[1]**3
        output[4] = self.P[1]*self.Q[4] + 2*self.P[2]*self.Q[0]*self.Q[4] + 2*self.P[2]*self.Q[1]*self.Q[3] + self.P[2]*self.Q[2]**2 + 3*self.P[3]*self.Q[0]**2*self.Q[4] + 6*self.P[3]*self.Q[0]*self.Q[1]*self.Q[3] + 3*self.P[3]*self.Q[0]*self.Q[2]**2 + 3*self.P[3]*self.Q[1]**2*self.Q[2] + 4*self.P[4]*self.Q[0]**3*self.Q[4] + 12*self.P[4]*self.Q[0]**2*self.Q[1]*self.Q[3] + 6*self.P[4]*self.Q[0]**2*self.Q[2]**2 + 12*self.P[4]*self.Q[0]*self.Q[1]**2*self.Q[2] + self.P[4]*self.Q[1]**4 + 5*self.P[5]*self.Q[0]**4*self.Q[4] + 20*self.P[5]*self.Q[0]**3*self.Q[1]*self.Q[3] + 10*self.P[5]*self.Q[0]**3*self.Q[2]**2 + 30*self.P[5]*self.Q[0]**2*self.Q[1]**2*self.Q[2] + 5*self.P[5]*self.Q[0]*self.Q[1]**4
        output[5] = self.P[1]*self.Q[5] + 2*self.P[2]*self.Q[0]*self.Q[5] + 2*self.P[2]*self.Q[1]*self.Q[4] + 2*self.P[2]*self.Q[2]*self.Q[3] + 3*self.P[3]*self.Q[0]**2*self.Q[5] + 6*self.P[3]*self.Q[0]*self.Q[1]*self.Q[4] + 6*self.P[3]*self.Q[0]*self.Q[2]*self.Q[3] + 3*self.P[3]*self.Q[1]**2*self.Q[3] + 3*self.P[3]*self.Q[1]*self.Q[2]**2 + 4*self.P[4]*self.Q[0]**3*self.Q[5] + 12*self.P[4]*self.Q[0]**2*self.Q[1]*self.Q[4] + 12*self.P[4]*self.Q[0]**2*self.Q[2]*self.Q[3] + 12*self.P[4]*self.Q[0]*self.Q[1]**2*self.Q[3] + 12*self.P[4]*self.Q[0]*self.Q[1]*self.Q[2]**2 + 4*self.P[4]*self.Q[1]**3*self.Q[2] + 5*self.P[5]*self.Q[0]**4*self.Q[5] + 20*self.P[5]*self.Q[0]**3*self.Q[1]*self.Q[4] + 20*self.P[5]*self.Q[0]**3*self.Q[2]*self.Q[3] + 30*self.P[5]*self.Q[0]**2*self.Q[1]**2*self.Q[3] + 30*self.P[5]*self.Q[0]**2*self.Q[1]*self.Q[2]**2 + 20*self.P[5]*self.Q[0]*self.Q[1]**3*self.Q[2] + self.P[5]*self.Q[1]**5
        output[6] = 2*self.P[2]*self.Q[1]*self.Q[5] + 2*self.P[2]*self.Q[2]*self.Q[4] + self.P[2]*self.Q[3]**2 + 6*self.P[3]*self.Q[0]*self.Q[1]*self.Q[5] + 6*self.P[3]*self.Q[0]*self.Q[2]*self.Q[4] + 3*self.P[3]*self.Q[0]*self.Q[3]**2 + 3*self.P[3]*self.Q[1]**2*self.Q[4] + 6*self.P[3]*self.Q[1]*self.Q[2]*self.Q[3] + self.P[3]*self.Q[2]**3 + 12*self.P[4]*self.Q[0]**2*self.Q[1]*self.Q[5] + 12*self.P[4]*self.Q[0]**2*self.Q[2]*self.Q[4] + 6*self.P[4]*self.Q[0]**2*self.Q[3]**2 + 12*self.P[4]*self.Q[0]*self.Q[1]**2*self.Q[4] + 24*self.P[4]*self.Q[0]*self.Q[1]*self.Q[2]*self.Q[3] + 4*self.P[4]*self.Q[0]*self.Q[2]**3 + 4*self.P[4]*self.Q[1]**3*self.Q[3] + 6*self.P[4]*self.Q[1]**2*self.Q[2]**2 + 20*self.P[5]*self.Q[0]**3*self.Q[1]*self.Q[5] + 20*self.P[5]*self.Q[0]**3*self.Q[2]*self.Q[4] + 10*self.P[5]*self.Q[0]**3*self.Q[3]**2 + 30*self.P[5]*self.Q[0]**2*self.Q[1]**2*self.Q[4] + 60*self.P[5]*self.Q[0]**2*self.Q[1]*self.Q[2]*self.Q[3] + 10*self.P[5]*self.Q[0]**2*self.Q[2]**3 + 20*self.P[5]*self.Q[0]*self.Q[1]**3*self.Q[3] + 30*self.P[5]*self.Q[0]*self.Q[1]**2*self.Q[2]**2 + 5*self.P[5]*self.Q[1]**4*self.Q[2]
        output[7] = 2*self.P[2]*self.Q[2]*self.Q[5] + 2*self.P[2]*self.Q[3]*self.Q[4] + 6*self.P[3]*self.Q[0]*self.Q[2]*self.Q[5] + 6*self.P[3]*self.Q[0]*self.Q[3]*self.Q[4] + 3*self.P[3]*self.Q[1]**2*self.Q[5] + 6*self.P[3]*self.Q[1]*self.Q[2]*self.Q[4] + 3*self.P[3]*self.Q[1]*self.Q[3]**2 + 3*self.P[3]*self.Q[2]**2*self.Q[3] + 12*self.P[4]*self.Q[0]**2*self.Q[2]*self.Q[5] + 12*self.P[4]*self.Q[0]**2*self.Q[3]*self.Q[4] + 12*self.P[4]*self.Q[0]*self.Q[1]**2*self.Q[5] + 24*self.P[4]*self.Q[0]*self.Q[1]*self.Q[2]*self.Q[4] + 12*self.P[4]*self.Q[0]*self.Q[1]*self.Q[3]**2 + 12*self.P[4]*self.Q[0]*self.Q[2]**2*self.Q[3] + 4*self.P[4]*self.Q[1]**3*self.Q[4] + 12*self.P[4]*self.Q[1]**2*self.Q[2]*self.Q[3] + 4*self.P[4]*self.Q[1]*self.Q[2]**3 + 20*self.P[5]*self.Q[0]**3*self.Q[2]*self.Q[5] + 20*self.P[5]*self.Q[0]**3*self.Q[3]*self.Q[4] + 30*self.P[5]*self.Q[0]**2*self.Q[1]**2*self.Q[5] + 60*self.P[5]*self.Q[0]**2*self.Q[1]*self.Q[2]*self.Q[4] + 30*self.P[5]*self.Q[0]**2*self.Q[1]*self.Q[3]**2 + 30*self.P[5]*self.Q[0]**2*self.Q[2]**2*self.Q[3] + 20*self.P[5]*self.Q[0]*self.Q[1]**3*self.Q[4] + 60*self.P[5]*self.Q[0]*self.Q[1]**2*self.Q[2]*self.Q[3] + 20*self.P[5]*self.Q[0]*self.Q[1]*self.Q[2]**3 + 5*self.P[5]*self.Q[1]**4*self.Q[3] + 10*self.P[5]*self.Q[1]**3*self.Q[2]**2
        output[8] = 2*self.P[2]*self.Q[3]*self.Q[5] + self.P[2]*self.Q[4]**2 + 6*self.P[3]*self.Q[0]*self.Q[3]*self.Q[5] + 3*self.P[3]*self.Q[0]*self.Q[4]**2 + 6*self.P[3]*self.Q[1]*self.Q[2]*self.Q[5] + 6*self.P[3]*self.Q[1]*self.Q[3]*self.Q[4] + 3*self.P[3]*self.Q[2]**2*self.Q[4] + 3*self.P[3]*self.Q[2]*self.Q[3]**2 + 12*self.P[4]*self.Q[0]**2*self.Q[3]*self.Q[5] + 6*self.P[4]*self.Q[0]**2*self.Q[4]**2 + 24*self.P[4]*self.Q[0]*self.Q[1]*self.Q[2]*self.Q[5] + 24*self.P[4]*self.Q[0]*self.Q[1]*self.Q[3]*self.Q[4] + 12*self.P[4]*self.Q[0]*self.Q[2]**2*self.Q[4] + 12*self.P[4]*self.Q[0]*self.Q[2]*self.Q[3]**2 + 4*self.P[4]*self.Q[1]**3*self.Q[5] + 12*self.P[4]*self.Q[1]**2*self.Q[2]*self.Q[4] + 6*self.P[4]*self.Q[1]**2*self.Q[3]**2 + 12*self.P[4]*self.Q[1]*self.Q[2]**2*self.Q[3] + self.P[4]*self.Q[2]**4 + 20*self.P[5]*self.Q[0]**3*self.Q[3]*self.Q[5] + 10*self.P[5]*self.Q[0]**3*self.Q[4]**2 + 60*self.P[5]*self.Q[0]**2*self.Q[1]*self.Q[2]*self.Q[5] + 60*self.P[5]*self.Q[0]**2*self.Q[1]*self.Q[3]*self.Q[4] + 30*self.P[5]*self.Q[0]**2*self.Q[2]**2*self.Q[4] + 30*self.P[5]*self.Q[0]**2*self.Q[2]*self.Q[3]**2 + 20*self.P[5]*self.Q[0]*self.Q[1]**3*self.Q[5] + 60*self.P[5]*self.Q[0]*self.Q[1]**2*self.Q[2]*self.Q[4] + 30*self.P[5]*self.Q[0]*self.Q[1]**2*self.Q[3]**2 + 60*self.P[5]*self.Q[0]*self.Q[1]*self.Q[2]**2*self.Q[3] + 5*self.P[5]*self.Q[0]*self.Q[2]**4 + 5*self.P[5]*self.Q[1]**4*self.Q[4] + 20*self.P[5]*self.Q[1]**3*self.Q[2]*self.Q[3] + 10*self.P[5]*self.Q[1]**2*self.Q[2]**3
        output[9] = 2*self.P[2]*self.Q[4]*self.Q[5] + 6*self.P[3]*self.Q[0]*self.Q[4]*self.Q[5] + 6*self.P[3]*self.Q[1]*self.Q[3]*self.Q[5] + 3*self.P[3]*self.Q[1]*self.Q[4]**2 + 3*self.P[3]*self.Q[2]**2*self.Q[5] + 6*self.P[3]*self.Q[2]*self.Q[3]*self.Q[4] + self.P[3]*self.Q[3]**3 + 12*self.P[4]*self.Q[0]**2*self.Q[4]*self.Q[5] + 24*self.P[4]*self.Q[0]*self.Q[1]*self.Q[3]*self.Q[5] + 12*self.P[4]*self.Q[0]*self.Q[1]*self.Q[4]**2 + 12*self.P[4]*self.Q[0]*self.Q[2]**2*self.Q[5] + 24*self.P[4]*self.Q[0]*self.Q[2]*self.Q[3]*self.Q[4] + 4*self.P[4]*self.Q[0]*self.Q[3]**3 + 12*self.P[4]*self.Q[1]**2*self.Q[2]*self.Q[5] + 12*self.P[4]*self.Q[1]**2*self.Q[3]*self.Q[4] + 12*self.P[4]*self.Q[1]*self.Q[2]**2*self.Q[4] + 12*self.P[4]*self.Q[1]*self.Q[2]*self.Q[3]**2 + 4*self.P[4]*self.Q[2]**3*self.Q[3] + 20*self.P[5]*self.Q[0]**3*self.Q[4]*self.Q[5] + 60*self.P[5]*self.Q[0]**2*self.Q[1]*self.Q[3]*self.Q[5] + 30*self.P[5]*self.Q[0]**2*self.Q[1]*self.Q[4]**2 + 30*self.P[5]*self.Q[0]**2*self.Q[2]**2*self.Q[5] + 60*self.P[5]*self.Q[0]**2*self.Q[2]*self.Q[3]*self.Q[4] + 10*self.P[5]*self.Q[0]**2*self.Q[3]**3 + 60*self.P[5]*self.Q[0]*self.Q[1]**2*self.Q[2]*self.Q[5] + 60*self.P[5]*self.Q[0]*self.Q[1]**2*self.Q[3]*self.Q[4] + 60*self.P[5]*self.Q[0]*self.Q[1]*self.Q[2]**2*self.Q[4] + 60*self.P[5]*self.Q[0]*self.Q[1]*self.Q[2]*self.Q[3]**2 + 20*self.P[5]*self.Q[0]*self.Q[2]**3*self.Q[3] + 5*self.P[5]*self.Q[1]**4*self.Q[5] + 20*self.P[5]*self.Q[1]**3*self.Q[2]*self.Q[4] + 10*self.P[5]*self.Q[1]**3*self.Q[3]**2 + 30*self.P[5]*self.Q[1]**2*self.Q[2]**2*self.Q[3] + 5*self.P[5]*self.Q[1]*self.Q[2]**4
        output[10] = self.P[2]*self.Q[5]**2 + 3*self.P[3]*self.Q[0]*self.Q[5]**2 + 6*self.P[3]*self.Q[1]*self.Q[4]*self.Q[5] + 6*self.P[3]*self.Q[2]*self.Q[3]*self.Q[5] + 3*self.P[3]*self.Q[2]*self.Q[4]**2 + 3*self.P[3]*self.Q[3]**2*self.Q[4] + 6*self.P[4]*self.Q[0]**2*self.Q[5]**2 + 24*self.P[4]*self.Q[0]*self.Q[1]*self.Q[4]*self.Q[5] + 24*self.P[4]*self.Q[0]*self.Q[2]*self.Q[3]*self.Q[5] + 12*self.P[4]*self.Q[0]*self.Q[2]*self.Q[4]**2 + 12*self.P[4]*self.Q[0]*self.Q[3]**2*self.Q[4] + 12*self.P[4]*self.Q[1]**2*self.Q[3]*self.Q[5] + 6*self.P[4]*self.Q[1]**2*self.Q[4]**2 + 12*self.P[4]*self.Q[1]*self.Q[2]**2*self.Q[5] + 24*self.P[4]*self.Q[1]*self.Q[2]*self.Q[3]*self.Q[4] + 4*self.P[4]*self.Q[1]*self.Q[3]**3 + 4*self.P[4]*self.Q[2]**3*self.Q[4] + 6*self.P[4]*self.Q[2]**2*self.Q[3]**2 + 10*self.P[5]*self.Q[0]**3*self.Q[5]**2 + 60*self.P[5]*self.Q[0]**2*self.Q[1]*self.Q[4]*self.Q[5] + 60*self.P[5]*self.Q[0]**2*self.Q[2]*self.Q[3]*self.Q[5] + 30*self.P[5]*self.Q[0]**2*self.Q[2]*self.Q[4]**2 + 30*self.P[5]*self.Q[0]**2*self.Q[3]**2*self.Q[4] + 60*self.P[5]*self.Q[0]*self.Q[1]**2*self.Q[3]*self.Q[5] + 30*self.P[5]*self.Q[0]*self.Q[1]**2*self.Q[4]**2 + 60*self.P[5]*self.Q[0]*self.Q[1]*self.Q[2]**2*self.Q[5] + 120*self.P[5]*self.Q[0]*self.Q[1]*self.Q[2]*self.Q[3]*self.Q[4] + 20*self.P[5]*self.Q[0]*self.Q[1]*self.Q[3]**3 + 20*self.P[5]*self.Q[0]*self.Q[2]**3*self.Q[4] + 30*self.P[5]*self.Q[0]*self.Q[2]**2*self.Q[3]**2 + 20*self.P[5]*self.Q[1]**3*self.Q[2]*self.Q[5] + 20*self.P[5]*self.Q[1]**3*self.Q[3]*self.Q[4] + 30*self.P[5]*self.Q[1]**2*self.Q[2]**2*self.Q[4] + 30*self.P[5]*self.Q[1]**2*self.Q[2]*self.Q[3]**2 + 20*self.P[5]*self.Q[1]*self.Q[2]**3*self.Q[3] + self.P[5]*self.Q[2]**5
        output[11] = 3*self.P[3]*self.Q[1]*self.Q[5]**2 + 6*self.P[3]*self.Q[2]*self.Q[4]*self.Q[5] + 3*self.P[3]*self.Q[3]**2*self.Q[5] + 3*self.P[3]*self.Q[3]*self.Q[4]**2 + 12*self.P[4]*self.Q[0]*self.Q[1]*self.Q[5]**2 + 24*self.P[4]*self.Q[0]*self.Q[2]*self.Q[4]*self.Q[5] + 12*self.P[4]*self.Q[0]*self.Q[3]**2*self.Q[5] + 12*self.P[4]*self.Q[0]*self.Q[3]*self.Q[4]**2 + 12*self.P[4]*self.Q[1]**2*self.Q[4]*self.Q[5] + 24*self.P[4]*self.Q[1]*self.Q[2]*self.Q[3]*self.Q[5] + 12*self.P[4]*self.Q[1]*self.Q[2]*self.Q[4]**2 + 12*self.P[4]*self.Q[1]*self.Q[3]**2*self.Q[4] + 4*self.P[4]*self.Q[2]**3*self.Q[5] + 12*self.P[4]*self.Q[2]**2*self.Q[3]*self.Q[4] + 4*self.P[4]*self.Q[2]*self.Q[3]**3 + 30*self.P[5]*self.Q[0]**2*self.Q[1]*self.Q[5]**2 + 60*self.P[5]*self.Q[0]**2*self.Q[2]*self.Q[4]*self.Q[5] + 30*self.P[5]*self.Q[0]**2*self.Q[3]**2*self.Q[5] + 30*self.P[5]*self.Q[0]**2*self.Q[3]*self.Q[4]**2 + 60*self.P[5]*self.Q[0]*self.Q[1]**2*self.Q[4]*self.Q[5] + 120*self.P[5]*self.Q[0]*self.Q[1]*self.Q[2]*self.Q[3]*self.Q[5] + 60*self.P[5]*self.Q[0]*self.Q[1]*self.Q[2]*self.Q[4]**2 + 60*self.P[5]*self.Q[0]*self.Q[1]*self.Q[3]**2*self.Q[4] + 20*self.P[5]*self.Q[0]*self.Q[2]**3*self.Q[5] + 60*self.P[5]*self.Q[0]*self.Q[2]**2*self.Q[3]*self.Q[4] + 20*self.P[5]*self.Q[0]*self.Q[2]*self.Q[3]**3 + 20*self.P[5]*self.Q[1]**3*self.Q[3]*self.Q[5] + 10*self.P[5]*self.Q[1]**3*self.Q[4]**2 + 30*self.P[5]*self.Q[1]**2*self.Q[2]**2*self.Q[5] + 60*self.P[5]*self.Q[1]**2*self.Q[2]*self.Q[3]*self.Q[4] + 10*self.P[5]*self.Q[1]**2*self.Q[3]**3 + 20*self.P[5]*self.Q[1]*self.Q[2]**3*self.Q[4] + 30*self.P[5]*self.Q[1]*self.Q[2]**2*self.Q[3]**2 + 5*self.P[5]*self.Q[2]**4*self.Q[3]
        output[12] = 3*self.P[3]*self.Q[2]*self.Q[5]**2 + 6*self.P[3]*self.Q[3]*self.Q[4]*self.Q[5] + self.P[3]*self.Q[4]**3 + 12*self.P[4]*self.Q[0]*self.Q[2]*self.Q[5]**2 + 24*self.P[4]*self.Q[0]*self.Q[3]*self.Q[4]*self.Q[5] + 4*self.P[4]*self.Q[0]*self.Q[4]**3 + 6*self.P[4]*self.Q[1]**2*self.Q[5]**2 + 24*self.P[4]*self.Q[1]*self.Q[2]*self.Q[4]*self.Q[5] + 12*self.P[4]*self.Q[1]*self.Q[3]**2*self.Q[5] + 12*self.P[4]*self.Q[1]*self.Q[3]*self.Q[4]**2 + 12*self.P[4]*self.Q[2]**2*self.Q[3]*self.Q[5] + 6*self.P[4]*self.Q[2]**2*self.Q[4]**2 + 12*self.P[4]*self.Q[2]*self.Q[3]**2*self.Q[4] + self.P[4]*self.Q[3]**4 + 30*self.P[5]*self.Q[0]**2*self.Q[2]*self.Q[5]**2 + 60*self.P[5]*self.Q[0]**2*self.Q[3]*self.Q[4]*self.Q[5] + 10*self.P[5]*self.Q[0]**2*self.Q[4]**3 + 30*self.P[5]*self.Q[0]*self.Q[1]**2*self.Q[5]**2 + 120*self.P[5]*self.Q[0]*self.Q[1]*self.Q[2]*self.Q[4]*self.Q[5] + 60*self.P[5]*self.Q[0]*self.Q[1]*self.Q[3]**2*self.Q[5] + 60*self.P[5]*self.Q[0]*self.Q[1]*self.Q[3]*self.Q[4]**2 + 60*self.P[5]*self.Q[0]*self.Q[2]**2*self.Q[3]*self.Q[5] + 30*self.P[5]*self.Q[0]*self.Q[2]**2*self.Q[4]**2 + 60*self.P[5]*self.Q[0]*self.Q[2]*self.Q[3]**2*self.Q[4] + 5*self.P[5]*self.Q[0]*self.Q[3]**4 + 20*self.P[5]*self.Q[1]**3*self.Q[4]*self.Q[5] + 60*self.P[5]*self.Q[1]**2*self.Q[2]*self.Q[3]*self.Q[5] + 30*self.P[5]*self.Q[1]**2*self.Q[2]*self.Q[4]**2 + 30*self.P[5]*self.Q[1]**2*self.Q[3]**2*self.Q[4] + 20*self.P[5]*self.Q[1]*self.Q[2]**3*self.Q[5] + 60*self.P[5]*self.Q[1]*self.Q[2]**2*self.Q[3]*self.Q[4] + 20*self.P[5]*self.Q[1]*self.Q[2]*self.Q[3]**3 + 5*self.P[5]*self.Q[2]**4*self.Q[4] + 10*self.P[5]*self.Q[2]**3*self.Q[3]**2
        output[13] = 3*self.P[3]*self.Q[3]*self.Q[5]**2 + 3*self.P[3]*self.Q[4]**2*self.Q[5] + 12*self.P[4]*self.Q[0]*self.Q[3]*self.Q[5]**2 + 12*self.P[4]*self.Q[0]*self.Q[4]**2*self.Q[5] + 12*self.P[4]*self.Q[1]*self.Q[2]*self.Q[5]**2 + 24*self.P[4]*self.Q[1]*self.Q[3]*self.Q[4]*self.Q[5] + 4*self.P[4]*self.Q[1]*self.Q[4]**3 + 12*self.P[4]*self.Q[2]**2*self.Q[4]*self.Q[5] + 12*self.P[4]*self.Q[2]*self.Q[3]**2*self.Q[5] + 12*self.P[4]*self.Q[2]*self.Q[3]*self.Q[4]**2 + 4*self.P[4]*self.Q[3]**3*self.Q[4] + 30*self.P[5]*self.Q[0]**2*self.Q[3]*self.Q[5]**2 + 30*self.P[5]*self.Q[0]**2*self.Q[4]**2*self.Q[5] + 60*self.P[5]*self.Q[0]*self.Q[1]*self.Q[2]*self.Q[5]**2 + 120*self.P[5]*self.Q[0]*self.Q[1]*self.Q[3]*self.Q[4]*self.Q[5] + 20*self.P[5]*self.Q[0]*self.Q[1]*self.Q[4]**3 + 60*self.P[5]*self.Q[0]*self.Q[2]**2*self.Q[4]*self.Q[5] + 60*self.P[5]*self.Q[0]*self.Q[2]*self.Q[3]**2*self.Q[5] + 60*self.P[5]*self.Q[0]*self.Q[2]*self.Q[3]*self.Q[4]**2 + 20*self.P[5]*self.Q[0]*self.Q[3]**3*self.Q[4] + 10*self.P[5]*self.Q[1]**3*self.Q[5]**2 + 60*self.P[5]*self.Q[1]**2*self.Q[2]*self.Q[4]*self.Q[5] + 30*self.P[5]*self.Q[1]**2*self.Q[3]**2*self.Q[5] + 30*self.P[5]*self.Q[1]**2*self.Q[3]*self.Q[4]**2 + 60*self.P[5]*self.Q[1]*self.Q[2]**2*self.Q[3]*self.Q[5] + 30*self.P[5]*self.Q[1]*self.Q[2]**2*self.Q[4]**2 + 60*self.P[5]*self.Q[1]*self.Q[2]*self.Q[3]**2*self.Q[4] + 5*self.P[5]*self.Q[1]*self.Q[3]**4 + 5*self.P[5]*self.Q[2]**4*self.Q[5] + 20*self.P[5]*self.Q[2]**3*self.Q[3]*self.Q[4] + 10*self.P[5]*self.Q[2]**2*self.Q[3]**3
        output[14] = 3*self.P[3]*self.Q[4]*self.Q[5]**2 + 12*self.P[4]*self.Q[0]*self.Q[4]*self.Q[5]**2 + 12*self.P[4]*self.Q[1]*self.Q[3]*self.Q[5]**2 + 12*self.P[4]*self.Q[1]*self.Q[4]**2*self.Q[5] + 6*self.P[4]*self.Q[2]**2*self.Q[5]**2 + 24*self.P[4]*self.Q[2]*self.Q[3]*self.Q[4]*self.Q[5] + 4*self.P[4]*self.Q[2]*self.Q[4]**3 + 4*self.P[4]*self.Q[3]**3*self.Q[5] + 6*self.P[4]*self.Q[3]**2*self.Q[4]**2 + 30*self.P[5]*self.Q[0]**2*self.Q[4]*self.Q[5]**2 + 60*self.P[5]*self.Q[0]*self.Q[1]*self.Q[3]*self.Q[5]**2 + 60*self.P[5]*self.Q[0]*self.Q[1]*self.Q[4]**2*self.Q[5] + 30*self.P[5]*self.Q[0]*self.Q[2]**2*self.Q[5]**2 + 120*self.P[5]*self.Q[0]*self.Q[2]*self.Q[3]*self.Q[4]*self.Q[5] + 20*self.P[5]*self.Q[0]*self.Q[2]*self.Q[4]**3 + 20*self.P[5]*self.Q[0]*self.Q[3]**3*self.Q[5] + 30*self.P[5]*self.Q[0]*self.Q[3]**2*self.Q[4]**2 + 30*self.P[5]*self.Q[1]**2*self.Q[2]*self.Q[5]**2 + 60*self.P[5]*self.Q[1]**2*self.Q[3]*self.Q[4]*self.Q[5] + 10*self.P[5]*self.Q[1]**2*self.Q[4]**3 + 60*self.P[5]*self.Q[1]*self.Q[2]**2*self.Q[4]*self.Q[5] + 60*self.P[5]*self.Q[1]*self.Q[2]*self.Q[3]**2*self.Q[5] + 60*self.P[5]*self.Q[1]*self.Q[2]*self.Q[3]*self.Q[4]**2 + 20*self.P[5]*self.Q[1]*self.Q[3]**3*self.Q[4] + 20*self.P[5]*self.Q[2]**3*self.Q[3]*self.Q[5] + 10*self.P[5]*self.Q[2]**3*self.Q[4]**2 + 30*self.P[5]*self.Q[2]**2*self.Q[3]**2*self.Q[4] + 5*self.P[5]*self.Q[2]*self.Q[3]**4
        output[15] = self.P[3]*self.Q[5]**3 + 4*self.P[4]*self.Q[0]*self.Q[5]**3 + 12*self.P[4]*self.Q[1]*self.Q[4]*self.Q[5]**2 + 12*self.P[4]*self.Q[2]*self.Q[3]*self.Q[5]**2 + 12*self.P[4]*self.Q[2]*self.Q[4]**2*self.Q[5] + 12*self.P[4]*self.Q[3]**2*self.Q[4]*self.Q[5] + 4*self.P[4]*self.Q[3]*self.Q[4]**3 + 10*self.P[5]*self.Q[0]**2*self.Q[5]**3 + 60*self.P[5]*self.Q[0]*self.Q[1]*self.Q[4]*self.Q[5]**2 + 60*self.P[5]*self.Q[0]*self.Q[2]*self.Q[3]*self.Q[5]**2 + 60*self.P[5]*self.Q[0]*self.Q[2]*self.Q[4]**2*self.Q[5] + 60*self.P[5]*self.Q[0]*self.Q[3]**2*self.Q[4]*self.Q[5] + 20*self.P[5]*self.Q[0]*self.Q[3]*self.Q[4]**3 + 30*self.P[5]*self.Q[1]**2*self.Q[3]*self.Q[5]**2 + 30*self.P[5]*self.Q[1]**2*self.Q[4]**2*self.Q[5] + 30*self.P[5]*self.Q[1]*self.Q[2]**2*self.Q[5]**2 + 120*self.P[5]*self.Q[1]*self.Q[2]*self.Q[3]*self.Q[4]*self.Q[5] + 20*self.P[5]*self.Q[1]*self.Q[2]*self.Q[4]**3 + 20*self.P[5]*self.Q[1]*self.Q[3]**3*self.Q[5] + 30*self.P[5]*self.Q[1]*self.Q[3]**2*self.Q[4]**2 + 20*self.P[5]*self.Q[2]**3*self.Q[4]*self.Q[5] + 30*self.P[5]*self.Q[2]**2*self.Q[3]**2*self.Q[5] + 30*self.P[5]*self.Q[2]**2*self.Q[3]*self.Q[4]**2 + 20*self.P[5]*self.Q[2]*self.Q[3]**3*self.Q[4] + self.P[5]*self.Q[3]**5
        output[16] = 4*self.P[4]*self.Q[1]*self.Q[5]**3 + 12*self.P[4]*self.Q[2]*self.Q[4]*self.Q[5]**2 + 6*self.P[4]*self.Q[3]**2*self.Q[5]**2 + 12*self.P[4]*self.Q[3]*self.Q[4]**2*self.Q[5] + self.P[4]*self.Q[4]**4 + 20*self.P[5]*self.Q[0]*self.Q[1]*self.Q[5]**3 + 60*self.P[5]*self.Q[0]*self.Q[2]*self.Q[4]*self.Q[5]**2 + 30*self.P[5]*self.Q[0]*self.Q[3]**2*self.Q[5]**2 + 60*self.P[5]*self.Q[0]*self.Q[3]*self.Q[4]**2*self.Q[5] + 5*self.P[5]*self.Q[0]*self.Q[4]**4 + 30*self.P[5]*self.Q[1]**2*self.Q[4]*self.Q[5]**2 + 60*self.P[5]*self.Q[1]*self.Q[2]*self.Q[3]*self.Q[5]**2 + 60*self.P[5]*self.Q[1]*self.Q[2]*self.Q[4]**2*self.Q[5] + 60*self.P[5]*self.Q[1]*self.Q[3]**2*self.Q[4]*self.Q[5] + 20*self.P[5]*self.Q[1]*self.Q[3]*self.Q[4]**3 + 10*self.P[5]*self.Q[2]**3*self.Q[5]**2 + 60*self.P[5]*self.Q[2]**2*self.Q[3]*self.Q[4]*self.Q[5] + 10*self.P[5]*self.Q[2]**2*self.Q[4]**3 + 20*self.P[5]*self.Q[2]*self.Q[3]**3*self.Q[5] + 30*self.P[5]*self.Q[2]*self.Q[3]**2*self.Q[4]**2 + 5*self.P[5]*self.Q[3]**4*self.Q[4]
        output[17] = 4*self.P[4]*self.Q[2]*self.Q[5]**3 + 12*self.P[4]*self.Q[3]*self.Q[4]*self.Q[5]**2 + 4*self.P[4]*self.Q[4]**3*self.Q[5] + 20*self.P[5]*self.Q[0]*self.Q[2]*self.Q[5]**3 + 60*self.P[5]*self.Q[0]*self.Q[3]*self.Q[4]*self.Q[5]**2 + 20*self.P[5]*self.Q[0]*self.Q[4]**3*self.Q[5] + 10*self.P[5]*self.Q[1]**2*self.Q[5]**3 + 60*self.P[5]*self.Q[1]*self.Q[2]*self.Q[4]*self.Q[5]**2 + 30*self.P[5]*self.Q[1]*self.Q[3]**2*self.Q[5]**2 + 60*self.P[5]*self.Q[1]*self.Q[3]*self.Q[4]**2*self.Q[5] + 5*self.P[5]*self.Q[1]*self.Q[4]**4 + 30*self.P[5]*self.Q[2]**2*self.Q[3]*self.Q[5]**2 + 30*self.P[5]*self.Q[2]**2*self.Q[4]**2*self.Q[5] + 60*self.P[5]*self.Q[2]*self.Q[3]**2*self.Q[4]*self.Q[5] + 20*self.P[5]*self.Q[2]*self.Q[3]*self.Q[4]**3 + 5*self.P[5]*self.Q[3]**4*self.Q[5] + 10*self.P[5]*self.Q[3]**3*self.Q[4]**2
        output[18] = 4*self.P[4]*self.Q[3]*self.Q[5]**3 + 6*self.P[4]*self.Q[4]**2*self.Q[5]**2 + 20*self.P[5]*self.Q[0]*self.Q[3]*self.Q[5]**3 + 30*self.P[5]*self.Q[0]*self.Q[4]**2*self.Q[5]**2 + 20*self.P[5]*self.Q[1]*self.Q[2]*self.Q[5]**3 + 60*self.P[5]*self.Q[1]*self.Q[3]*self.Q[4]*self.Q[5]**2 + 20*self.P[5]*self.Q[1]*self.Q[4]**3*self.Q[5] + 30*self.P[5]*self.Q[2]**2*self.Q[4]*self.Q[5]**2 + 30*self.P[5]*self.Q[2]*self.Q[3]**2*self.Q[5]**2 + 60*self.P[5]*self.Q[2]*self.Q[3]*self.Q[4]**2*self.Q[5] + 5*self.P[5]*self.Q[2]*self.Q[4]**4 + 20*self.P[5]*self.Q[3]**3*self.Q[4]*self.Q[5] + 10*self.P[5]*self.Q[3]**2*self.Q[4]**3
        output[19] = 4*self.P[4]*self.Q[4]*self.Q[5]**3 + 20*self.P[5]*self.Q[0]*self.Q[4]*self.Q[5]**3 + 20*self.P[5]*self.Q[1]*self.Q[3]*self.Q[5]**3 + 30*self.P[5]*self.Q[1]*self.Q[4]**2*self.Q[5]**2 + 10*self.P[5]*self.Q[2]**2*self.Q[5]**3 + 60*self.P[5]*self.Q[2]*self.Q[3]*self.Q[4]*self.Q[5]**2 + 20*self.P[5]*self.Q[2]*self.Q[4]**3*self.Q[5] + 10*self.P[5]*self.Q[3]**3*self.Q[5]**2 + 30*self.P[5]*self.Q[3]**2*self.Q[4]**2*self.Q[5] + 5*self.P[5]*self.Q[3]*self.Q[4]**4
        output[20] = self.P[4]*self.Q[5]**4 + 5*self.P[5]*self.Q[0]*self.Q[5]**4 + 20*self.P[5]*self.Q[1]*self.Q[4]*self.Q[5]**3 + 20*self.P[5]*self.Q[2]*self.Q[3]*self.Q[5]**3 + 30*self.P[5]*self.Q[2]*self.Q[4]**2*self.Q[5]**2 + 30*self.P[5]*self.Q[3]**2*self.Q[4]*self.Q[5]**2 + 20*self.P[5]*self.Q[3]*self.Q[4]**3*self.Q[5] + self.P[5]*self.Q[4]**5
        output[21] = 5*self.P[5]*self.Q[1]*self.Q[5]**4 + 20*self.P[5]*self.Q[2]*self.Q[4]*self.Q[5]**3 + 10*self.P[5]*self.Q[3]**2*self.Q[5]**3 + 30*self.P[5]*self.Q[3]*self.Q[4]**2*self.Q[5]**2 + 5*self.P[5]*self.Q[4]**4*self.Q[5]
        output[22] = 5*self.P[5]*self.Q[2]*self.Q[5]**4 + 20*self.P[5]*self.Q[3]*self.Q[4]*self.Q[5]**3 + 10*self.P[5]*self.Q[4]**3*self.Q[5]**2
        output[23] = 5*self.P[5]*self.Q[3]*self.Q[5]**4 + 10*self.P[5]*self.Q[4]**2*self.Q[5]**3
        output[24] = 5*self.P[5]*self.Q[4]*self.Q[5]**4
        output[25] = self.P[5]*self.Q[5]**5
